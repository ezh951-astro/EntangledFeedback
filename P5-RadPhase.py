...
for j,(RunName) in enumerate(hd.runlabels):
    if j in ActiveRunIDs:
        ...
        if j==4:
            ...
            axs['histy'].hist(itemp_Main_RIGEL, bins=space_temp, density=True, histtype='step', orientation='horizontal', color='C2') # needed to correct for bug in GasTemp function input units - np.log10(1.989e43)
        ...
        #hd.axisstyle(axs['histx'])
        #hd.axisstyle(axs['histy'])
        #hd.axisstyle(axs['scatter'])
        ...
        axs['histx'].tick_params(labelsize=11,direction='in')
        axs['histy'].tick_params(labelsize=11,direction='in')
        axs['scatter'].tick_params(labelsize=11,direction='in')

        axs['histx'].tick_params(which='minor',direction='in')
        axs['histy'].tick_params(which='minor',direction='in')

        axs['histx'].yaxis.set_ticks_position('both')
        axs['histy'].xaxis.set_ticks_position('both')

        fig.savefig('OfficialPlots/P5-RadPhase{}.png'.format(j),dpi=360)
        plt.show()
