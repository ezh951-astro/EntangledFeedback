...
for jSim,localdata in enumerate(LocalDatas):
    if jSim in [0]:
        ...
        if jSim==0:
            ...
            axsE[0].plot( dens_space, vtoE(30)*np.ones(50), color='black', linestyle='dashed', alpha=0.6 )
            axsE[0].text( 1.5, 46.17, r'$v=30 {\rm km/s}$', color='black', fontsize=12, ha='center', va='top', alpha=0.6 )

            axsE[0].fill_between( [3,MAXDENS], [EMIN,EMIN], [EMAX,EMAX], color='#add8e6', alpha=0.3 )
            axsE[1].fill_between( [3,MAXDENS], [0,0], [NSNMAX,NSNMAX], color='#add8e6', alpha=0.3 )
            axsE[0].text( 2.95, 42.1, r'SF Thresh.', color='#6d98a6', rotation=90, ha='right', va='bottom', fontsize=12, alpha=0.95 )

            axsE[1].hist( idens_Main, color='red', bins=np.arange(MINDENS,MAXDENS,0.5), alpha=0.65, histtype='step', linewidth=3 )

            axsE[0].set_ylabel(r'$\Delta e$ [erg/${\rm M}_{\odot}$]',fontsize=14.5)
            axsE[1].set_ylabel('No. SNe',fontsize=14.5)
            figE.supxlabel(r'Local Density [${\rm cm}^{-3}$]',fontsize=14.5)
...
figE.savefig('GalFresca/4.png',dpi=300)
plt.show()