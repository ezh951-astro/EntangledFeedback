...
for jSim,localdata in enumerate(LocalDatas):
    if jSim in [0,1]:
        ...
        if jSim==0:
            ...
            axsE[0].plot( dens_space, np.log10( 1.e51/1680./(10.**dens_space)**(-0.26) )*np.ones(50), color='purple', linestyle='dashed', alpha=0.5 )
            axsE[0].text( 2.5, 48.62, 'S-T Transition\n KO+15', color='purple', fontsize=12, ha='center', va='bottom', alpha=0.8 )
            ...
            axsE[0].plot( dens_space, vtoE(30)*np.ones(50), color='black', linestyle='dashed', alpha=0.4 )
            axsE[0].text( 1.5, 46.17, r'$v=30 {\rm km/s}$', color='black', fontsize=12, ha='center', va='top', alpha=0.4 )
            ...
            axsE[0].text( 2.95, 42.1, r'SF Thresh.', color='pink', rotation=90, ha='right', va='bottom' )
            ...
            axsE[0].set_ylabel(r'Avg. $\Delta e$ [erg/${\rm M}_{\odot}$]',fontsize=14.5)
            axsE[1].set_ylabel('No. SNe',fontsize=14.5)
            figE.supxlabel(r'Avg. Local Density [${\rm cm}^{-3}$]',fontsize=14.5)
